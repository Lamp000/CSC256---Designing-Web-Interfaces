...
function addGameToPlaylist(game,playlist){
    // typical transactions and stuff
    const transaction=db.transaction("playlists","readwrite");
    const store=transaction.objectStore("playlists");
    const request=store.get(playlist);
    request.onsuccess=function(){
        // this takes the list from the playlist db, and adds the game(ID) to the list. I can then reference that array when accessing playlist.html.
